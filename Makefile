...
.PHONY: build build-all test lint check e2e package verify-release clean help
...
## e2e: Live end-to-end tests against the real ip.thc.org API (network required)
e2e: build
...
## (rdns-lookup-vX.Y.Z-<os>-<arch>.<ext>).
package: build-all
...
BREW_KIND := formula
BREW_DESC := Look up an IP's domains, a domain's subdomains, and reverse CNAMEs

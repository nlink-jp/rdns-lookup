...
- **Every tool schema is closed and the decoder enforces it.** `closeSchemas` sets `additionalProperties: false` on every tool's top-level schema as one pass over the finished list (organization ADR-021 §10), so a tool added later cannot forget it, and `TestEveryToolSchemaIsClosed` reads the schemas back off `tools/list`. The server half is `decodeArgs` (`json.Decoder.DisallowUnknownFields`), which every handler decodes through, including the argument-less ones. **A call carrying an argument a tool does not declare now fails, naming the field, instead of being silently ignored** — the deliberate behaviour change ADR-021 §4 requires, made because the schema binds validating clients only and a caller speaking JSON-RPC directly validates nothing. A misspelled `limit` used to fall back to the default while the result read as the bounded set asked for, and `limit` is this server's only bound on a response. `TestUnknownArgumentIsRefusedByName` replaced `TestUnknownArgumentIsAcceptedByTheServer`, which had pinned the opposite.
- **Each lookup tool decodes its own argument struct** (`rdnsArgs` / `subdomainsArgs` / `cnamesArgs`, sharing an embedded `commonArgs`). They were one union struct, which with strict decoding would have kept a hole the schemas do not have: `ip_address` sent to `lookup_subdomains` decodes into a declared field, so the decoder accepts it and the tool ignores it. Add a new argument to the tool that declares it, not to `commonArgs`, unless all three take it. Do not merge them back.

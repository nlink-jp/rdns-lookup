...
- **Every tool schema is closed, but only the client side is enforced.** `closeSchemas` sets `additionalProperties: false` on every tool's top-level schema as one pass over the finished list (organization ADR-021 §10), so a tool added later cannot forget it, and `TestEveryToolSchemaIsClosed` reads the schemas back off `tools/list`. The server half is **missing**: `toolLookup` decodes with plain `json.Unmarshal`, so an unknown argument that arrives anyway is accepted and ignored — a misspelled `limit` silently falls back to the default. `TestUnknownArgumentIsAcceptedByTheServer` pins that, deliberately, so the gap is visible in code. Adding `DisallowUnknownFields` turns a silently-ignored argument into an error for existing callers, so it is a behaviour change to decide on its own, not a side effect of the schema work; when it is made, that test should fail and be replaced by its opposite.
